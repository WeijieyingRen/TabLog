...
class ConjunctionLayer(nn.Module):
    ...
    def forward(self, x):
        #print ('---ConjunctionLayer forward---')
        #pdb.set_trace()
        if self.use_not:
            x = torch.cat((x, 1 - x), dim=1)
    ...
    def binarized_forward(self, x):
        #print ('---ConjunctionLayer discretize forward---')
        if self.use_not:
            x = torch.cat((x, 1 - x), dim=1)
...
class DisjunctionLayer(nn.Module):
    ...
    def forward(self, x):
        #print ('----- DisjunctionLayer forward -----')
        #pdb.set_trace()
        if self.use_not:
            x = torch.cat((x, 1 - x), dim=1)
        #pdb.set_trace()
    ...
    def binarized_forward(self, x):
        #print ('----- DisjunctionLayer binarized forward -----')
        if self.use_not:
            x = torch.cat((x, 1 - x), dim=1)
        Wb = Binarize.apply(self.W - THRESHOLD)
        #pdb.set_trace()
...
class UnionLayer(nn.Module):
    ...
    def get_rule_description(self, prev_rule_name, wrap=False):
        ...
        # len(self.rule_name) = 12
        #pdb.set_trace()
        print (' --- ending get rule description ---')
